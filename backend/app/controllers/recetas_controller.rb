...
class RecetasController < ApplicationController
  ...
  def receta_params
    params.require(:receta).permit(:nombre, :fase, :ph_objetivo, :ec_objetivo, :notas, :activa,
  ...
  def serialize(r, con_uso: false)
    base = {
      ...
      # Para 1 litro: con esto la pantalla calcula cualquier volumen sin volver a pedir.
      por_litro: r.receta_items.map { |it| { insumo_id: it.insumo_id, dosis: it.dosis } },
    }
  ...
  def uso_de(r)
    ...
    lotes = registros.map(&:lote).compact.uniq
    {
      ...
      costo_total_ars: registros.sum { |x| x.nutricion.to_h['costo_ars'].to_f }.round(2),
    }
  end
end
