# backend/app/controllers/informe_semestral_controller.rb
class InformeSemestralController < ApplicationController
  ...
  # Params: año (default: año actual), semestre (1 o 2, default: semestre actual)
  def show
    ...
    render json: {
      ...
      resumen_geneticas: resumen_geneticas(club),
  ...
  def datos_club(club)
    {
      ...
      sedes_reprocann: club.sedes.where(declarada_reprocann: true).map { |s|
        { nombre: s.nombre, tipo: s.tipo_label, direccion: [s.direccion, s.ciudad, s.provincia].compact.reject(&:empty?).join(", ") }
      },
    }
  end
  ...
  def datos_produccion(club, desde, hasta)
    ...
    plantas_ids  = Plant.where(lote_id: lotes_ids).pluck(:id)

    # Plantas activas al momento del informe
    total_plantas      = Plant.where(lote_id: lotes_ids)
                              .where.not(state: %w[cosechada descartada])
                              .count
  ...
  def resumen_geneticas(club)
    club.geneticas.where(activa: true).map do |g|
      plantas = Plant.joins(:lote).where(lotes: { club_id: club.id }, genetica_id: g.id)
                     .where.not(state: %w[cosechada descartada]).count
      {
        nombre:        g.nombre,
        tipo:          g.tipo,
        thc:           g.thc&.to_f,
        cbd:           g.cbd&.to_f,
        plantas_activas: plantas,
      }
