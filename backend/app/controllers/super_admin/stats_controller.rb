class SuperAdmin::StatsController < SuperAdmin::BaseController
  ...
  def metricas
    ...
    render json: {
      ...
      total_pacientes:       Paciente.count,
      total_dispensaciones_mes: Dispensacion.no_canceladas.where('fecha_dispensacion >= ?', inicio_mes).count,
      total_gramos_dispensados_mes: Dispensacion.no_canceladas.where('fecha_dispensacion >= ?', inicio_mes).sum(:cantidad).to_f,
