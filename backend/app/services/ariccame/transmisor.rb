module Ariccame
  # Transmite un registro a ARICCAME. La integración REAL con su API
  ...
  # POST a ARICCAME en `enviar_real` con `@registro.payload`.
  class Transmisor
    ...
    # Simulación: un código con formato plausible y respuesta marcada como simulada.
    def simular_envio
      codigo = "SIM-#{Time.current.strftime('%Y%m')}-#{format('%06d', @registro.id)}"
      [codigo, { simulado: true, mensaje: 'Aceptado (simulado, sin envío real a ARICCAME)' }]
    end

    # TODO: POST real a la API de ARICCAME con @registro.payload cuando haya credenciales.
    def enviar_real
      raise 'Integración real con ARICCAME no configurada (ARICCAME_SIMULAR=false sin API)'
    end
  end
end
