...
RSpec.describe 'POST /lotes/mover', type: :request do
  ...
  describe 'el lote toma la fase de la sala destino' do
    ...
    it 'un lote ENRAIZANDO movido a una sala de vegetativo sigue enraizando' do
      lote = create(:lote, club: club, sala: sala_mixta, estado: 'enraizado')
    ...
    # A una sala de floración NO se lo deja entrar: el propagador tiene su propia humedad,
    # pero la luz se la da la sala, y enraizando hace falta MÁS tiempo de luz, no 12/12.
    it 'no lo deja entrar a una sala de floración' do
      ...
      expect(JSON.parse(response.body)['error']).to match(/no prende/i)
  ...
  describe 'en tanda' do
    it 'mueve varios lotes de una y devuelve solo los que cambiaron de fase' do
      ...
      otro    = create(:lote, club: club, sala: sala_vege, estado: 'vegetativo')
      sign_in_as(admin)

      mover([en_vege.id, otro.id], sala_flora)

      body = JSON.parse(response.body)
      expect(body['movidos']).to eq(2)
      expect(body['cambios_de_fase'].map { |c| c['codigo'] }).to match_array([en_vege.codigo, otro.codigo])
      expect(en_vege.reload.estado).to eq('floracion')
    end

    # En tanda no se mueve nada a medias: si uno de los elegidos no puede entrar, se avisa
    # cuál es y no se toca ninguno.
    it 'rechaza la tanda entera si alguno no puede entrar, y dice cuál' do
      en_vege    = create(:lote, club: club, sala: sala_vege, estado: 'vegetativo')
      ...
      expect(response).to have_http_status(:unprocessable_entity)
      body = JSON.parse(response.body)
      expect(body['lotes_rechazados'].map { |l| l['codigo'] }).to eq([enraizando.codigo])
      expect(en_vege.reload.sala_id).to eq(sala_vege.id)
    end
