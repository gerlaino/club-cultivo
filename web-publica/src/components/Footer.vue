  ...
  <footer class="wp-footer">
    <div class="wp-footer-inner">
      ...
      <div class="wp-footer-brand-col">
        ...
        <div v-if="club?.legal_name" class="wp-footer-legal">{{ club.legal_name }}</div>
        ...
        <div class="wp-footer-socials">
          <a v-if="club?.instagram_url" :href="club.instagram_url" target="_blank" class="wp-footer-social">
          ...
          <a v-if="club?.facebook_url" :href="club.facebook_url" target="_blank" class="wp-footer-social">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
              <path d="M18 2h-3a5 5 0 0 0-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 0 1 1-1h3z"/>
            </svg>
          </a>
          <a v-if="club?.whatsapp" :href="`https://wa.me/${club.whatsapp.replace(/\D/g,'')}`" target="_blank" class="wp-footer-social">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
      ...
      <div class="wp-footer-col">
        ...
        <div v-if="club?.address" class="wp-footer-contact-item">
          <svg class="wp-footer-contact-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/></svg>
          {{ club.address }}, {{ club.city }}
        </div>
        <div v-if="club?.phone" class="wp-footer-contact-item">
          <svg class="wp-footer-contact-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.69 13 19.79 19.79 0 0 1 1.62 4.36 2 2 0 0 1 3.59 2h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L7.91 9.91a16 16 0 0 0 6.16 6.16l.92-.92a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0 1 22 16.92z"/></svg>
          {{ club.phone }}
        </div>
        <div v-if="club?.email" class="wp-footer-contact-item">
          <svg class="wp-footer-contact-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg>
          {{ club.email }}
        </div>
      </div>
...
.wp-footer {
  background: #080c08;
  border-top: 1px solid rgba(109, 190, 138, 0.1);
  margin-top: auto;
}
...
.wp-footer-social:hover {
  border-color: rgba(109, 190, 138, 0.3);
  color: #6dbe8a;
  background: rgba(109, 190, 138, 0.09);
}

.wp-footer-social svg {
  width: 15px;
  height: 15px;
}
