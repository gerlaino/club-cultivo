    ...
    <div v-if="show" class="mt-overlay" @click.self="$emit('cerrar')">
      <div class="mt-panel">
        ...
        <div class="mt-header">
          <div class="mt-header-left">
            ...
            <div>
              ...
              <p class="mt-subtitle" v-if="form.sala_id">
                {{ salas.find(s => s.id == form.sala_id)?.nombre || '' }}
              </p>
            </div>
          </div>
        ...
        <div class="mt-body">
          ...
          <!-- ── Descripción ── -->
          <div class="mt-field">
            <label class="mt-label">Descripción <span class="mt-optional">opcional</span></label>
            <textarea
              v-model="form.descripcion"
              class="mt-input mt-textarea"
              rows="2"
          ...
          <!-- ── Repetición ── -->
          <div class="mt-field">
            <label class="mt-label">Repetición <span class="mt-optional">opcional</span></label>
            <label class="mt-toggle">
              <input type="checkbox" v-model="conRepeticion" class="mt-toggle-input" />
              <div class="mt-toggle-track"><div class="mt-toggle-thumb"></div></div>
              <span class="mt-toggle-label">Repetir esta tarea</span>
            </label>

            <div v-if="conRepeticion" class="mt-repeticion-panel">
              <div class="mt-row mt-row--3">
                ...
                <div class="mt-field">
                  ...
                  <input
                    v-model.number="repeticion.veces"
          ...
          <div class="mt-row">
            ...
            <div class="mt-field">
              <label class="mt-label">Lote</label>
          ...
          <div v-if="puedeAsignar" class="mt-field">
            <label class="mt-label">Asignar a</label>
            <select v-model="form.asignada_a_id" class="mt-input">
...
const PRIORIDADES = [
  { value: 'baja',    label: 'Baja',    dot: '🟢', color: '#2e7d32', bg: '#e8f5e9' },
  { value: 'normal',  label: 'Normal',  dot: '🔵', color: '#1565c0', bg: '#e3f2fd' },
  { value: 'alta',    label: 'Alta',    dot: '🟠', color: '#e65100', bg: '#fff3e0' },
  { value: 'urgente', label: 'Urgente', dot: '🔴', color: '#b71c1c', bg: '#ffebee' },
]
...
const fechasGeneradas = computed(() => {
  ...
  const maxVeces = repeticion.value.veces || 4
...
watch(() => props.show, (val) => {
  ...
  form.value = props.tareaInicial ? {
    ...
    tipo:             props.tareaInicial.tipo || 'riego',
    prioridad:        props.tareaInicial.prioridad || 'normal',
    asignada_a_id:    props.tareaInicial.asignada_a?.id || '',
    sala_id:          props.tareaInicial.sala_id || props.tareaInicial.sala?.id || '',
    lote_id:          props.tareaInicial.lote_id || props.tareaInicial.lote?.id || '',
...
async function guardar() {
  ...
  const salaNombre = props.salas.find(s => s.id == form.value.sala_id)?.nombre || ''
  const payload = {
    ...form.value,
...
.mt-panel {
  ...
  box-shadow: 0 24px 60px rgba(27,94,32,.18);
...
.mt-header-icon {
  ...
  display: flex; align-items: center; justify-content: center;
  font-size: 1.3rem; flex-shrink: 0; transition: background .2s;
}
.mt-title { font-size: 1.05rem; font-weight: 700; color: #1a1a1a; margin: 0; }
.mt-subtitle { font-size: .75rem; color: #60725d; margin: .1rem 0 0; }
.mt-close {
  background: #e8f0e9; border: none;
  width: 32px; height: 32px; border-radius: 8px;
  cursor: pointer; display: flex; align-items: center; justify-content: center;
  color: #60725d; font-size: .8rem; transition: all .15s;
}
...
.mt-label {
  font-size: .72rem; font-weight: 700; color: #60725d;
  text-transform: uppercase; letter-spacing: .05em;
  display: flex; align-items: center; gap: .4rem;
}
.mt-label-sm { font-size: .72rem; font-weight: 600; color: #60725d; }
.mt-optional { font-size: .68rem; font-weight: 500; color: #94a3b8; text-transform: none; letter-spacing: 0; }
...
.mt-input:focus { outline: none; border-color: #1b5e20; background: #fff; }
...
.mt-input-suffix {
  ...
  padding: .65rem .85rem; font-size: .8rem; font-weight: 600; color: #60725d;
...
.mt-prioridad {
  ...
  padding: .45rem .85rem; border: 1.5px solid #d4e6d4;
...
.mt-toggle-track {
  width: 38px; height: 21px; background: #d4e6d4;
  border-radius: 999px; position: relative; transition: background .2s; flex-shrink: 0;
}
.mt-toggle-input:checked + .mt-toggle-track { background: #1b5e20; }
.mt-toggle-thumb {
...
.mt-toggle-input:checked + .mt-toggle-track .mt-toggle-thumb { left: 20px; }
...
.mt-fechas-preview { margin-top: .75rem; }
...
.mt-fecha-chip {
  font-size: .7rem; font-weight: 600; color: #1b5e20;
...
.mt-btn-ghost {
  background: transparent; color: #60725d;
  border: 1.5px solid #d4e6d4; padding: .65rem 1.25rem;
  border-radius: 10px; font-size: .875rem; font-weight: 500;
  cursor: pointer; transition: all .15s;
}
.mt-btn-ghost:hover { background: #e8f0e9; color: #1b5e20; border-color: #66bb6a; }
.mt-btn-primary {
...
.mt-spinner {
  ...
  animation: mt-spin .6s linear infinite;
}
