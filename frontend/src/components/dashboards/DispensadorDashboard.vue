<template>
  <div class="dd">
    ...
    <!-- ── Mi semana ───────────────────────────────────────────────────────────
         Las tareas asignadas a esta persona, día por día. El endpoint ya scopea por asignado
         (`asignadas_a(current_user.id)`), así que acá no hay nada que filtrar. -->
    <div class="dd__section">
      <div class="dd__section-hd">
        <h2 class="dd__section-title">Mi semana</h2>
        <RouterLink to="/tareas" class="dd__section-link">Ver tareas →</RouterLink>
      </div>
      <div v-if="cargandoSemana" class="dd__empty">Cargando…</div>
      <div v-else-if="!totalTareasSemana" class="dd__empty">Sin tareas asignadas esta semana.</div>
      <div v-else class="dd__sem">
        <RouterLink
          v-for="dia in diasSemana" :key="dia.fecha" to="/tareas"
          class="dd__sem-dia" :class="{ 'dd__sem-dia--hoy': dia.esHoy, 'dd__sem-dia--vacio': !dia.tareas?.length }"
        >
          <span class="dd__sem-nombre">{{ dia.dia_semana?.slice(0, 3) }}</span>
          <span class="dd__sem-num">{{ new Date(dia.fecha + 'T00:00:00').getDate() }}</span>
          <span v-if="dia.pendientes" class="dd__sem-badge">{{ dia.pendientes }}</span>
          <span v-else class="dd__sem-ok">·</span>
        </RouterLink>
      </div>
    </div>
...
onMounted(async () => {
  ...
  try {
    const { data } = await getTareasSemana()
    semana.value = data
  } catch { /* la semana es complemento: si falla, el resto del inicio sirve igual */ }
  finally { cargandoSemana.value = false }
})
</script>
...
.dd__section-link:hover { text-decoration: underline; }
.dd__sem { display: grid; grid-template-columns: repeat(7, 1fr); gap: var(--sp-2); }
.dd__sem-dia { display: flex; flex-direction: column; align-items: center; gap: .15rem; padding: var(--sp-2) 0; border: 1.5px solid var(--c-ink-200); border-radius: 10px; text-decoration: none; background: #fff; }
.dd__sem-dia--hoy { border-color: #1b5e20; background: #f0fdf4; }
.dd__sem-dia--vacio { opacity: .55; }
.dd__sem-nombre { font-size: var(--fs-11); text-transform: uppercase; letter-spacing: .04em; color: var(--c-ink-500); }
.dd__sem-num { font-size: var(--fs-16); font-weight: 700; color: var(--c-ink-900); font-variant-numeric: tabular-nums; }
.dd__sem-badge { font-size: var(--fs-11); font-weight: 800; color: #fff; background: #b45309; border-radius: 999px; padding: 0 .35rem; min-width: 18px; text-align: center; }
.dd__sem-ok { font-size: var(--fs-14); color: var(--c-ink-300); line-height: 1; }
