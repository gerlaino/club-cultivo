...
const FORMA_LBL_PRINT = { flor_seca:'Flor seca', flor:'Flor', aceite:'Aceite', hash:'Hash', extracto:'Extracto', comestible:'Comestible' }
...
async function imprimirEtiqueta(d) {
  ...
  try {
    ...
    const forma   = FORMA_LBL_PRINT[d.stock?.forma_producto] || d.stock?.forma_producto || ''
    const cant    = `${d.cantidad}${d.stock?.unidad || 'g'}`
    const fecha   = fmtDate(d.fecha_dispensacion)
    const lote    = d.stock?.lote?.codigo || d.lote_codigo || ''

    const html = `<!DOCTYPE html><html lang="es"><head><meta charset="UTF-8">
<title>Etiqueta ${_esc(gen)}</title>
<style>
  @page { margin: 8mm; }
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  <div class="lbl">
    ...
    <div class="lbl-data">
      <div class="lbl-club">${logo ? `<img src="${_esc(logo)}">` : ''}${_esc(clubN)}</div>
      <div class="lbl-gen">${_esc(gen)}</div>
      <div class="lbl-meta">${_esc(forma)} · ${_esc(cant)}${lote ? ' · ' + _esc(lote) : ''}</div>
      <div class="lbl-meta">${_esc(fecha)}</div>
