...
const voz = useReconocimientoVoz({
  alTerminar: (dicho, porPedido) => {
    ...
    // Sólo se procesa si la persona dio por terminado. Cuando el motor se rinde solo (se quedó
    // sin reintentos, o se cayó la red a mitad de frase) lo dictado queda escrito y a la vista
    // con el botón "Procesar" al lado. Mandarlo igual gastaría una llamada de IA —que se mide y
    // se cobra— para registrar media frase, y el registro malo después hay que borrarlo a mano.
